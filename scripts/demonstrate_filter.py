...
def run():
    ...
    for idx, (job, score) in enumerate(raw_results, 1):
        ...
        print(
            f"   [Job Specs: Visa={job.visa_sponsorship}, Max Salary={job.max_salary}, Remote={job.remote_ok}]"
        )
    ...
    match_results = db.scalars(
        select(MatchResult).where(MatchResult.candidate_id == candidate_id)
    ).all()
