...
def test_upload_valid_pdf() -> None:
    ...
    response = client.post("/upload-resume", files={"file": ("test.pdf", file, "application/pdf")})
...
def test_upload_duplicate_pdf() -> None:
    ...
    response1 = client.post(
        "/upload-resume", files={"file": ("duplicate.pdf", file1, "application/pdf")}
    )
    ...
    response2 = client.post(
        "/upload-resume", files={"file": ("duplicate.pdf", file2, "application/pdf")}
    )
...
def test_upload_invalid_mime_type() -> None:
    ...
    response = client.post("/upload-resume", files={"file": ("test.pdf", file, "text/plain")})
...
def test_upload_invalid_extension() -> None:
    ...
    response = client.post("/upload-resume", files={"file": ("test.txt", file, "application/pdf")})
    assert response.status_code == 400
    assert "Only PDF files are allowed" in response.json()["detail"]
...
def test_upload_exceeds_size_limit() -> None:
    ...
    response = client.post("/upload-resume", files={"file": ("large.pdf", file, "application/pdf")})
    assert response.status_code == 400
